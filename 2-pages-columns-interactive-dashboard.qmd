---
title: "An interactive Dashboard with Penguins Data"
format: 
    dashboard:
        orientation: columns # defining primary orientation
---

```{r}
#| include: false

library(palmerpenguins)
library(ggplot2)
library(dplyr)
library(leaflet) # interactive maps
library(plotly) # interactive plots
```

# Page 1 {orientation="rows"}

## Row 1 {height="10%"}

::: {.card}
This is an dashboard with 2 pages and with default orientation set to columns.

Page 1 plays with different orientation options (rows and columns). And Page 2 presents values boxes and an interactive map using `leaflet`.
:::


## Row 2 {height="70%"}

### Column 1

```{r}
#| title: Penguin Scatterplot


ggplot(data = penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
    geom_point(aes(color = species)) +
    scale_color_manual(values = c(
        "Adelie" = "purple2",
        "Chinstrap" = "orange",
        "Gentoo" = "cyan4"
    )) +
    labs(
        title = NULL,
        x = "Flipper Length (mm)",
        y = "Body Mass (g)",
        color = "Species"
    ) +
    theme_minimal()
```


### Column 2



```{r}
#| title: Bill Depth Distribution of Chinstrap

penguins %>%
    filter(species == "Chinstrap") %>%
    ggplot(aes(x = bill_depth_mm)) +
    geom_histogram(fill = "orange") +
    labs(title = NULL,
         x = "Bill Depth (mm)",
         y = NULL) +
    theme_minimal()
```



```{r}
#| title: Penguin Species on Biscoe Island

# subsetted penguins data frame
biscoe_sp <- penguins %>% 
    filter(island == "Biscoe") %>% 
    group_by(species) %>% 
    summarize(count = n())
    
# bar plot 
ggplot(data = biscoe_sp, aes(x = species, y = count)) +
    geom_col(aes(fill = species)) +
    scale_fill_manual(values = c(
        "Adelie" = "purple2",
        "Gentoo" = "cyan4"
    )) +
    labs(title = NULL,
         x = NULL,
         y = "Number of species",
         fill = "Species") +
    theme_minimal() 
```


# Page 2

## Column 1 {width=30%}


```{r}
#| content: valuebox
#| title: "Total Penguins"

list(
  icon = "hash",
  color = "warning",
  value = nrow(penguins)
)
```



```{r}
#| content: valuebox
#| title: "Penguin Species"

list(
  icon = "feather",
  color = "light",
  value = length(unique(penguins$species)))
```


## Column 2 {width=70%}

::: {.card title="About Palmer Station" height=25%}
[Palmer Station](https://pallter.marine.rutgers.edu/) is a site part of the Long-Term Ecological Research (LTER) Network. It is one of the three United States research stations located in Antarctica. 

It is also where the data from the [Palmer Penguins](https://allisonhorst.github.io/palmerpenguins/) R package was collected. The penguin data was collected and made available by [Dr. Kristen Gorman](https://www.uaf.edu/cfos/people/faculty/detail/kristen-gorman.php).


:::



```{r}

leaflet() %>%
    addProviderTiles(providers$OpenStreetMap) %>% 
    addMarkers(lng = -64.05384063012775,
               lat = -64.77413239299318,
               popup = "Palmer Station")
```
